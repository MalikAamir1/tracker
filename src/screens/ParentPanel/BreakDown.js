...
const Breakdown = ({route, navigation}) => {
  ...
  const teamId = route.params.teamId; // Replace 'your_team_id' with the actual team ID
  const getAllBudgetOfTeam = async () => {
    const url = `${localhostUrl}expense/team/${teamId}/budget`;
  ...
  const getAllExpensesOfTeam = async () => {
    try {
      ...
      data.map(item => {
        ...
        spent += parseInt(item.amount); // Add the
      });
      setTotalSpent(spent);
          ...
          <View>
            <View
              ...
              <View>
                <TouchableOpacity
                  onPress={() =>
                    navigation.navigate('HistoryOfExpenses', teamId)
                  }>
                  <Text
                    style={{
                      color: '#312F33',
            ...
            <View
              ...
              <FlatList
                data={data && data.length > 0 ? data.slice(0, 2) : []}
